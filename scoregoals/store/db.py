"""scoregoals.store.db — FROZEN sqlite + JSON-file persistence.

Single sqlite database at <data_dir>/dayloop.db (the filename keeps its legacy
"dayloop.db" name — it is live data and renaming the file would orphan the
existing database), plus human-readable JSON copies under data/timeline/ and
data/reports/. Stdlib-only.
"""
...
def load_timeline(config: Config, date: str) -> DayTimeline | None:
    ...
    scoregoals keeps every timeline in BOTH sqlite (the `timelines` row) and a
    ...
    if json_path.is_file():
        try:
            ...
            print(f"scoregoals: warning: could not read {json_path} ({exc})", file=sys.stderr)
    ...
    try:
        ...
        print(f"scoregoals: warning: timeline heal write failed for {date} ({exc})", file=sys.stderr)

    print(
        f"scoregoals: timeline divergence for {date} healed — kept newer {chosen_src} copy"

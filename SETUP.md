...
dayloop is a personal, local-first activity tracker for macOS: it captures what
...
- **[screenpipe](https://screenpi.pe) desktop app** — *optional but recommended*
  ...
  for personal use. dayloop only detects it at `http://localhost:3030` and never
...
```sh
.venv/bin/python -m dayloop config set gemini_api_key <your-key>   # set
.venv/bin/python -m dayloop config get gemini_api_key              # -> set / not set
.venv/bin/python -m dayloop config set gemini_api_key ""           # clear
```
...
- **Menu bar app says "engine not found — set path in Settings".** The app
  ...
  repo directory (the folder containing `dayloop/cli.py` and `.venv`), or the
  `.venv/bin/dayloop` binary directly. The app also honors `$DAYLOOP_BIN`.
- **`dayloop doctor` shows screenpipe ✗.** Install the desktop app from

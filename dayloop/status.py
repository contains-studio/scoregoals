"""dayloop.status — assemble the live snapshot the menu bar app polls.

`dayloop status --json` prints ONE JSON object (schema_version=1, documented in
...
def _warn(warnings: list[str], msg: str) -> None:
    warnings.append(msg)
    print(f"[dayloop.status] {msg}", file=sys.stderr)
...
def build_json(config: Config, date: str) -> str:
    ...
    try:
        ...
        print(f"[dayloop.status] fatal ({exc})", file=sys.stderr)
        return json.dumps(
            {
                "schema_version": SCHEMA_VERSION,
                "date": date,

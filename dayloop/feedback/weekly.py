...
def generate(week_start: str, config: Config) -> str:
    ...
    if not timelines:
        return (
            f"# dayloop — Weekly synthesis {week_start} → {week_end}\n\n"
            "_No timelines captured for this week — run `dayloop capture <date>` "
            "(or `dayloop mock`) first._\n"
        )
    ...
    lines: list[str] = [f"# dayloop — Weekly synthesis {week_start} → {week_end}", ""]
    lines.append(

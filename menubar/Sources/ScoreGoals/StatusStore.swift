...
/// Polls `scoregoals status --json` on a (live-adjustable) cadence, decodes the
...
final class StatusStore: ObservableObject {
    @Published private(set) var status: ScoreGoalsStatus? = nil
    @Published private(set) var lastError: String? = nil
    @Published private(set) var lastUpdated: Date? = nil
    @Published private(set) var isRefreshing: Bool = false

    /// Effective app settings from `config --json` (populated on start + Settings open).
    @Published private(set) var config: ScoreGoalsConfig? = nil
    ...
    private var client: ScoreGoalsClient
    private let workQueue = DispatchQueue(label: "scoregoals.status.poll", qos: .userInitiated)
    private var timer: Timer?

    init(client: ScoreGoalsClient = .resolveDefault(),
    ...
    nonisolated static func resolvedRefreshSeconds() -> TimeInterval {
        if let raw = scoreGoalsEnv("REFRESH_SECONDS", ProcessInfo.processInfo.environment),
    ...
    /// False when no scoregoals engine could be located — the UI shows a clear
    ...
    func refresh() {
        ...
        workQueue.async { [weak self] in
            ...
            var decodedStatus: ScoreGoalsStatus? = nil
            var errorMessage: String? = nil
            switch result {
            case .success(let data):
                do {
                    decodedStatus = try JSONDecoder().decode(ScoreGoalsStatus.self, from: data)
                } catch {
    ...
    func loadConfig() {
        Task {
            ...
            if case .success(let text) = result,
               ...
               let cfg = try? JSONDecoder().decode(ScoreGoalsConfig.self, from: data) {
                self.config = cfg
    ...
    func setEnginePath(_ path: String) {
        ...
        if trimmed.isEmpty {
            UserDefaults.standard.removeObject(forKey: ScoreGoalsDefaults.enginePathKey)
        } else {
            UserDefaults.standard.set(trimmed, forKey: ScoreGoalsDefaults.enginePathKey)
        }
